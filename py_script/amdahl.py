...
#
def plotStrongScaling(csv_filename = "../data/kip_openMP_strongScaling.csv", min_relative_time = 0.05, 
    ...
    for (image_dim, kernel_dim), subgroup in grouped.groupby(["ImageDimension", "KernelDimension"]):
        ...
        # Diagram: linear fit estimation
        plt.figure(figsize=(7,5))
        plt.scatter(x, y, label="Experimental data", color="blue", s=60)
        x_fit = np.linspace(0, 1, 100).reshape(-1, 1)
        y_fit = model.predict(x_fit)
        plt.plot(x_fit, y_fit, "--", color="red", label=f"Linear fit (f ≈ {f_est:.3f})")
